...
specified_directory = r'E:\Game\Chd\小草\心月狐\Log\AutoLog\D7FCD7FCD7D4C8BBC3C8A4B4D7FC'

# 要替换的字符串和替换后的字符串
old_strings = ['经验套装装',]  # 替换为你要查找的字符串
        ...
        if os.path.isfile(file):  # 确保是文件而不是文件夹
            ...
            for find_string in find_strings:
                if find_string in content:
                    ...
                    string = f"{str2:<20} {str3:<16} {str1:<10} |||---{find_string}"
                    # print(string)
                    strings.append(string)
            print(f"文件 {file} 检测完毕")


checkFile(specified_directory)
