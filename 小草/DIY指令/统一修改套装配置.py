...
# 指定目录路径
specified_directory = 'E:\Game\Chd\小草\小熊\data\Config\D7FCD7FCD7D4C8BBC3C8A4B4D7FC'  # 替换为你的目录路径

# 要替换的字符串和替换后的字符串
old_strings = ['经验套装装',]  # 替换为你要查找的字符串
new_strings = ['经验套装']  # 替换为你想要的新字符串
find_strings = ['经验套','格挡套','珊瑚手枪','珊瑚套']
    ...
    for file in files:
        if os.path.isfile(file):  # 确保是文件而不是文件夹
            ...
            if replacements_made:
                # 写入修改后的内容回原文件
                with open(file, 'w', encoding='utf-8') as f:  # 写入时使用'utf-8'编码
